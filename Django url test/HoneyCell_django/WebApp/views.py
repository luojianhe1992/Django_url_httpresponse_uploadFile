...
def url_test(request):
    print("in the url_test function")
...
@login_required
def url_test_2(request):
    print("in the url_test_2 function")
...
@login_required
def url_test_3(request):
    print("in the url_test3 function")
...
def upload_test(request):
    ...
    print("in the upload_test function")

    context = {}

    context['user'] = request.user
    context['task_name'] = request.POST['task_name']
    context['task_description'] = request.POST['task_description']
    context['task_docfile'] = request.FILES['docfile']
    ...
    print(context['task_docfile'])
    print("*" * 10)

    new_task_instance = Task(user = context['user'],
                             name = context['task_name'],
                             description = context['task_description'],
                             docfile = context['task_docfile'])

    new_task_instance.save()

    print("new_task_instance save")

    return HttpResponseRedirect(reverse('message'))


def show_files(request):
    print("in the show_files function")

    context = {}

    context['user'] = request.user

    current_user_files = Task.objects.filter(user = request.user)

    context['files'] = current_user_files

    return render(request, 'WebApp/show_files.html', context)